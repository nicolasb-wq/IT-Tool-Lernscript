...
def content_box(kind: str, text: str, width: float) -> Table:
    ...
    body = Paragraph(text, STYLES["box_body"])
...
def comparison_table(headers: list[str], rows: list[list[str]], width: float) -> Table:
    ...
    data = [[Paragraph(f"<b><font color='white'>{h}</font></b>", STYLES["box_body"]) for h in headers]]
    data += [[Paragraph(str(c), STYLES["box_body"]) for c in row] for row in rows]
