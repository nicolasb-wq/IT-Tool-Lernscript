...
def build_pdf(script: dict) -> bytes:
    ...
    flow = [
        Paragraph(st.safe_markup(script["titel"]), st.STYLES["title"]),
        Paragraph(st.safe_markup(script.get("untertitel", "")), st.STYLES["kicker"]),
        Spacer(1, 6 * mm),
    ]
    ...
    for i, kap in enumerate(script.get("kapitel", [])):
        ...
        for b in kap.get("bloecke", []):
            typ = b.get("typ")
            if typ == "h2":
                flow.append(Paragraph(st.safe_markup(b["text"]), st.STYLES["h2"]))
            elif typ == "text":
                flow.append(Paragraph(st.safe_markup(b["text"]), st.STYLES["body"]))
            elif typ == "bullets":
                for item in b["items"]:
                    flow.append(Paragraph(st.safe_markup(item), st.STYLES["bullet"], bulletText="•"))
